...
### 2️⃣ Undo / Redo 시스템 — 메모 편집 히스토리 기능
    ...
    Notion처럼 편집 상태 히스토리를 DB가 아닌 메모리 레벨에서 구현
    ...
    ➡ 실무에서 어려운 UX 기능을 직접 구현하여 문제 해결 능력을 증명

### 3️⃣ 블록 타입 확장 (멀티미디어 지원)

    Notion과 유사한 블록 구조를 확장하여 다양한 타입을 지원
...
### 4️⃣ 검색 + 태그 시스템 — 고급 UX 기능
    ...
    메모 전체에서 특정 단어를 빠르게 찾을 수 있도록 Full-Text Search(FTS) 기능을 추가
...
###6️⃣ Jetpack Compose 리팩토링 — 최신 안드로이드 개발 기술
    ...
    기존 XML 기반 RecyclerView UI를 Compose 기반으로 재구현 시도
...
유지보수성과 확장성을 고려한 Clean Architecture Lite 형태로 구성
